...
rem
rem Copyright (c) 2010-2014 Pivotal Software, Inc.  All rights reserved.
rem ---------------------------------------------------------------------------
rem version: 3.2.4.SR1
rem build date: 20170109163315
...
"%JAVA_HOME%\bin\java" %JAVA_OPTS% -Dtcs.version=3.2.4.SR1 -Dtcs.edition=standard -Djava.util.logging.config.file="%SCRIPT_DIR%\conf\commons-logging.properties" -Druntime.directory="%RUNTIME_DIR%" -Ddefault.instance.directory="%INSTANCE_DIR%" -classpath %CLASSPATH% com.springsource.tcruntime.instance.TcRuntimeAdmin %*
SET _RET=%ERRORLEVEL%
IF %_RET% EQU 1 (
    IF %ARG_COUNT% EQU 0 (
        pause
